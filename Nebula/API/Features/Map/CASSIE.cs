// -----------------------------------------------------------------------
// <copyright file=CASSIE.cs company="Nebula-Software-Development">
...
namespace Nebula.API.Features.Map
{
    ...
    public static class CASSIE
    {
        ...
        public static void SendMessage(string message, bool Hold = false, bool Noisy = true, bool Subtitles = false)
        {
            RespawnEffectsController.PlayCassieAnnouncement(message, Hold, Noisy, Subtitles);
        }
        ...
        public static void SendCustomCaptionMessage(string originalMessage, string subtitleTranslation,
            bool Hold = false, bool Noisy = true, bool Subtitles = true)
        {
            ...
            for (int i = 0; i < cassieMessages.Length; i++)
            {
                announcementBuilder.Append(
                    $"{subtitleTranslations[i].Replace(' ', ' ')}<size=0> {cassieMessages[i]} </size><split>");
            }
        ...
        public static void SendGlitchyMessage(string message, float glitchAmount, float jamChance)
        {
            NineTailedFoxAnnouncer.singleton.ServerOnlyAddGlitchyPhrase(message, glitchAmount, jamChance);
        }
        ...
        public static void ConvertTeam(Team team, string unitName)
        {
        ...
        public static float TimeToSpeak(string text, bool rawNumber = false, float speed = 1)
        {
            return NineTailedFoxAnnouncer.singleton.CalculateDuration(text, rawNumber, speed);
        }
    }
}