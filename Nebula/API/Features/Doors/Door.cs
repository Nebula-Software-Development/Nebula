// -----------------------------------------------------------------------
// <copyright file=Door.cs company="Nebula-Software-Development">
...
namespace Nebula.API.Features.Doors
{
    ...
    public class Door
    {
        ...
        /// </summary>
        public DoorType Type { get; } = DoorType.UnknownDoor;
        ...
        public void PermissionDenied(Player player, byte colliderId)
        {
            Base.PermissionsDenied(player.ReferenceHub, colliderId);
        }

        /// <summary>
        ///     Triggers a door action.
        /// </summary>
        public void TriggerDoorAction(DoorVariant door, DoorAction action, Player player)
        {
            TriggerDoorAction(door, action, player.ReferenceHub);
        }

        /// <summary>
        ///     Triggers a door action.
        /// </summary>
        public void TriggerDoorAction(DoorVariant door, DoorAction action, ReferenceHub player)
        {
            DoorEvents.TriggerAction(door, action, player);
        }
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public static Door GetType(DoorType type)
        {
        ...
        {
            ChangeDoorLock(DoorLockingType.None);
        }
        ...
        public void UnLockLater(float timeToWait, DoorLockingType typeToUnlock)
        {
            Base.UnlockLater(timeToWait, (DoorLockReason)typeToUnlock);
        }
        ...
        {
            if (door.GetComponent<DoorNametagExtension>() is not null &&
                nameToDoorType.TryGetValue(door.GetComponent<DoorNametagExtension>().GetName, out DoorType doorType))
            {
                return doorType;
            }
            ...
            return door.name.GetSubstringBeforeCharacter(' ') switch
            {
                ...
                {
                    RoomType.LczAirlock => Base.GetComponentInParent<AirlockController>() != null
                        ? DoorType.Airlock
