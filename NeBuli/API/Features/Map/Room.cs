// -----------------------------------------------------------------------
// <copyright file=Room.cs company="Nebula-Software-Development">
...
namespace Nebula.API.Features.Map
{
    ...
    public class Room
    {
        ...
        /// </summary>
        /// <summary>
        ///     Gets the <see cref="MapGeneration.FacilityZone" /> of the room.
        /// </summary>
        ...
        {
            return RoomIdUtils.IsTheSameRoom(position, Position);
        }

        /// <summary>
        ///     Disables the room lights for the specified amount of time.
        /// </summary>
        public void DisableRoomLights(float duration)
        {
        ...
        {
            return Base.TryGetMainCoords(out coords);
        }
        ...
        /// <param name="zone"></param>
        /// <returns></returns>
        public static List<Room> GetRoomsInZone(FacilityZone zone)
        {
            return List.Where(room => room.Zone == zone).ToList();
        }
        ...
        {
            LightController.SetLights(state);
        }
        ...
        {
            return Dictionary.TryGetValue(identifier, out Room room) ? room : new Room(identifier);
        }

        /// <summary>
        ///     Gets the first room which <see cref="RoomType" /> is equal to the given <see cref="RoomType" />.
        /// </summary>
        public static Room GetType(RoomType type)
        {
            return List.FirstOrDefault(room => room.Type == type);
        }
        ...
        {
            Room room = Get(RoomIdUtils.RoomAtPositionRaycasts(position));
            if (room is not null)
            {
                return room;
            }
            ...
            switch (gameObject.name.GetSubstringBeforeCharacter(' '))
            {
                ...
                default:
                    if (gameObject.transform.parent != null &&
                        gameObject.transform.parent.name == "HCZ_EZ_Checkpoint (A)")
                    {
                        return RoomType.HczEzCheckpointA;
                    }

                    if (gameObject.transform.parent != null &&
                        gameObject.transform.parent.name == "HCZ_EZ_Checkpoint (B)")
                    {
