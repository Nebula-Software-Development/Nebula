...
{
    [HarmonyTranspiler]
    private static IEnumerable<CodeInstruction> OnChangingCamera(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
    {
        List<CodeInstruction> newInstructions = EventManager.CheckPatchInstructions<ChangingCamerasPatch>(117, instructions);
        ...
        newInstructions.InsertRange(index, new CodeInstruction[]
        {
           ...
           new(OpCodes.Callvirt, PropertyGetter(typeof(Scp079CurrentCameraSync), nameof(Scp079CurrentCameraSync.Owner))),
           new(OpCodes.Ldloc_0),
           new(OpCodes.Ldarg_0),
           ...
           new(OpCodes.Ldloc_S, changingCamera.LocalIndex),
           new(OpCodes.Callvirt, PropertyGetter(typeof(Scp079ChangingCameraEvent), nameof(Scp079ChangingCameraEvent.IsCancelled))),
           new(OpCodes.Brtrue_S, retLabel),
           new(OpCodes.Ldloc_S, changingCamera.LocalIndex),
           new(OpCodes.Callvirt, PropertyGetter(typeof(Scp079ChangingCameraEvent), nameof(Scp079ChangingCameraEvent.AuxDrain))),
           new(OpCodes.Stloc_0),
        });
